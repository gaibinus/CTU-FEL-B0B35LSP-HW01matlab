...
% 
% WARNING: Long tt may be very slow and/or memory demanding.
% 
% Examples:
...
% -> tt = '0011101011001010'
% 
% Author: Petter Källström, petter.kallstrom@gmail.se
% Date: June, 2012
...
function [Bins,inps,Nums,ott] = minTruthtable(tt, flags)
  ...
  doquine = false;
  if islogical(flags)
    verify = flags; % for compatibility with the old syntax.
  elseif ischar(flags)
    ...
    error('minTruthtable:flags', 'Wrong data type of flags: %s', class(flags));
  end
  ...
  assert(size(tt,2) == 2^N, 'minTruthtable:tt_size_mismatch', 'Error in minTruthtable: tt has length %d, which is not a power of two.', size(tt,2));
  assert(all(tt=='-' | tt=='0' | tt=='1'), 'minTruthtable:tt_char', 'Error in minTruthtable: Unexpected character in tt');
  assert(N < 16, 'minTruthtable:tt_too_big', 'Error in minTruthtable: tt has length 2^%d, wich is too long',N);
  ...
  MINC = 2000; % matrix increase size: Larger => faster for big tt, Smaller => slower for very big tt's.
  BinsNew = char('X'*ones(MINC,N));         % To prealloc an "empty" matrix (use 'X' for debug purpose)
  Bins2 = BinsNew;
  Covs2 = false(MINC,1);
  %Nums2 = int16(-ones(MINC,1));
  Nums2 = uint16(zeros(MINC,1));
  ...
  for n=1:N % Main iteration loop. Will break in the middle (unless tt = ones(2^N,1))
    ...
    for ix0 = 1:nVars1-1
      ...
      Nums10 = Nums1(ix0,:);
      Ones10 = Ones1(ix0);
      tmp = bitxor(Ones10, Ones1((ix0+1):end)); % bitwise version of Bins10 ~= Bins(<remaining>)
      isok = find(Dons10==Dons1((ix0+1):end) & (tmp==0 | bitand(tmp, tmp-1)==0))'+ix0;
      % Explanation: bitand(tmp, tmp-1) => 0 iff tmp=0 or a power of two.
      for ix1 = isok
        % now: Dons1(ix1) == Dons10, and Ones1(ix1) differs in at most one bit from Ones1(ix0)
  ...
  %           once, and that minimizes the cost.
  ...
  elseif ~isnan(dolevel)
    ...
    clear A vcnt ttEq1 cost nVars3
  else % The original Quine-McCluskey way
...
% This holds for all optimization functions below:
% Find boolean column vector
% * such that all(sum(ix,:),1)>1, i.e. all columns of A has non-zeros.
% * minimize cost = sum(cost(ix)).
% Stop searching if cost > maxcost (return an all-true vector).
...
function ix = binopt(A, cost, maxcost)
  ...
  % Now, find the row that covers most, and (later) "test" it on and off,
...
function ix = quickopt(A, cost)
  ...
  % Find those rows that _has_ to be included
  vcnt = sum(A,1); % count how many rows covers each column
  assert(all(vcnt > 0), 'minTruthtable:INTOPT:UNSOLVABLE');
  ix = any(A(:,vcnt==1),2); % logical index to rows in A
  ...
  while ~isempty(cuse) && ~isempty(ruse)
    ...
    [~,sel_row] = min(cost2); % indexing ruse, e.g. A(ruse(sel_row),...)
    
    % Move to covered terms
    ...
    ruse(sel_row) = [];       % remove
    % Removing the columns may result in empty rows in A(:,cuse), leading to
    % rows with cost2=inf. It's likely faster to leave them, than find them.
  end
end
...
%   Then minimize cost = inps + sum(abs(tt - ott) / (1-abs(tt-ott))).
...
function BIST()
  ...
  Ms = 3:8;
  cnts = [0, 2000, 1000, 300, 80, 9]; % 0 <=> 3^(2^M)
  for Mix=1:length(Ms), M=Ms(Mix); % number of variables
    ttn = ones(1,2^M); % ttn = numbered version of tt. 1 = first element in "digits"
    cnt = cnts(Mix); % number of tests.
    doall = ~cnt;
    if doall
      cnt = 3^(2^M);
      fprintf('| /== %d inputs (vec len=2^%d=%d, #tests = all 3^%d=%d) ==\n', M,M,2^M,2^M,cnt);
    else
      fprintf('| /== %d inputs (vec len=2^%d=%d, #tests=%d out of 3^%d=%.1e) ==\n', M,M,2^M,cnt,2^M,3^(2^M));
    end
    ...
    fprintf('| | Progress (done at 9): 0');
    next_i = round(cnt/9);
    for i=1:cnt
      if ~doall
        if i==1, ttn = ones(1,2^M); end
        if i==2, ttn = 3*ones(1,2^M); end
        if i>2, ttn = ceil(3*rand(1,2^M)); end
      end
      tt = digits(ttn);
      t=tic; [~, inpe]=minTruthtable(tt,['v' flags{1}]); times(1) = times(1)+toc(t); sums(1)=sums(1)+inpe;
      for j=2:numel(flags)
        t=tic; [~, inp]=minTruthtable(tt,['v' flags{j}]); times(j)=times(j)+toc(t);
        sums(j) = sums(j) + (inp-inpe);
        maxs(j) = max(maxs(j), (inp-inpe));
        cntdiffs(j) = cntdiffs(j) + (inp>inpe);
        if strcmp(flags{1},'e')
          assert(inpe<=inp, 'minTruthtable:BIST:UNOPT:UNEXPECTED', 'inpe=%d should be < inp(%s)=%d.',inpe,flags{j},inp);
        end
      end
      if doall
        ...
        assert(any(ttn>1), 'minTruthtable:Count:UNEXPECTED');
      end
      % print progress indicator
      if i==next_i
        foo = round(9*i/cnt); % 1, 2, ..., 9=at end
        fprintf('%d',foo);
        next_i = round((foo+1)*cnt/9);
      end
    end
